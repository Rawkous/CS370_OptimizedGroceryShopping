...
def api_items():
    """
    Return distinct product names from the `product` table (since price_history is empty).
    Optional query params:
      - ?q=<substring> (case-insensitive match via collation)
    ...
    conditions = ["p.name IS NOT NULL", "p.name <> ''"]
    params = []

    if q:
        # Use collation-driven case-insensitivity and allow index on name
    ...
    sql = f"""
        SELECT TRIM(p.name) AS item_name
        FROM product p
        {where_sql}
